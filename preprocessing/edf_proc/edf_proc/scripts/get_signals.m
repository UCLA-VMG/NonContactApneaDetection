...
function [signals, signals_sys_ts] = get_signals(file_path, signal_names)
    ...
    for i = 1:length(signal_names) % for every signal name we need
        ...
        if signal_name == "SignalLabel15_AIR_flow"
            signal_name = "AIR-flow";
        end
        if signal_name == "SignalLabel14_PTAF"
            signal_name = "PTAF";
        end
        if signal_name == "SignalLabel16_CHEST"
            signal_name = "CHEST";
        end
        if signal_name == "SignalLabel17_ABD"
            signal_name = "ABD";
        end
        if signal_name == "SignalLabel19_Wireless"
            signal_name = "Wireless";
        end
