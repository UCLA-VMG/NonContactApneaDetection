...
def add_apnea_dicts(html_path, signals_raw):
    ...
    for table_id in range(2, pap[0][0] + 1,1):
        ...
        #print(table_array)
        if table_id == pap[0][0]:
            apnea_epoch_list.append(table_array[ap[0][1] + 1:pap[0][1],0])
            apnea_start_list.append(table_array[ap[0][1] + 1:pap[0][1],1])
            # print(table_array[ap[0][1] + 1:pap[0][1],1])
            apnea_duration_list.append(table_array[ap[0][1] + 1:pap[0][1],2])
        if table_id < pap[0][0]:
            # print(table_array[ap[0][1] + 1:,1])
            apnea_epoch_list.append(table_array[ap[0][1] + 1:,0])
            apnea_start_list.append(table_array[ap[0][1] + 1:,1])
            apnea_duration_list.append(table_array[ap[0][1] + 1:,2])
    ...
    ap_ep_ar = np.concatenate(apnea_epoch_list,axis = 0).astype(int)
    ap_start_ar = np.concatenate(apnea_start_list, axis = 0)
    ap_dur_ar = np.concatenate(apnea_duration_list, axis = 0)
    ...
    sa = np.zeros((len(signals_raw["ABD"][1])))
    ...
    all_vitals_new["Sleep_Apnea"] = (sa, signals_raw["ABD"][1])
